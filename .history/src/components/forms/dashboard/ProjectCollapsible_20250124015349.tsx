...
const ProjectCollapsible: React.FC<{
  projects: any[]
  onCreateProject: () => Promise<any> // Updated to handle async creation
  ...
  const [projectList, setProjectList] = useState<any[]>(projects) // Maintain local project list state
  ...
  const handleCreateProject = async () => {
    try {
      const newProject = await onCreateProject() // Wait for the new project to be created
      setProjectList((prevProjects) => [newProject, ...prevProjects]) // Add the new project to the top of the list
    } catch (error) {
