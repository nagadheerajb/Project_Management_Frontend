...
import { formatDate } from "@/utils/format-date" // Import the formatDate utility
...
const CompanyDetailsSection: React.FC<{
  ...
  useEffect(() => {
    if (selectedCompany) {
      getCompanyDetails(selectedCompany).then((data) => {
        const augmentedData = {
          ...data,
          displayName: `${data.firstName || ""} ${data.lastName || ""}`.trim() || "N/A"
        }
        setCompanyDetails(augmentedData)
      })
    }
  }, [selectedCompany])
        ...
        <CardContent>
          <div className="grid gap-4">
            ...
            <div className="flex items-center space-x-4">
              ...
              <span className="font-mono text-sm">{companyDetails.displayName}</span>
            </div>
          </div>
        </CardContent>
      </Card>
